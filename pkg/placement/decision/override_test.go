...
func TestApplyOverride(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedStatus:   OverrideStatusPending, // Becomes active immediately if no approval required
...
func TestFindMatchingPolicies(t *testing.T) {
	manager := &DecisionOverride{
		overridePolicies: make(map[string]*OverridePolicy),
	}

	// Create test policies
	productionPolicy := &OverridePolicy{
	...
	developmentPolicy := &OverridePolicy{
	...
	emergencyPolicy := &OverridePolicy{
	...
	// Add policies
	manager.overridePolicies["production"] = productionPolicy
	manager.overridePolicies["development"] = developmentPolicy
	manager.overridePolicies["emergency"] = emergencyPolicy
	manager.overridePolicies["disabled"] = disabledPolicy
	...
	tests := []struct {
	...
	}{
		{
			...
			spec: &OverrideSpec{Emergency: false},
			expectedPolicies: []string{"production-policy"},
			expectedCount:    1,
		},
		{
			name: "development placement",
			placement: &placementv1alpha1.WorkloadPlacement{
				ObjectMeta: metav1.ObjectMeta{
					Labels: map[string]string{
						"environment": "development",
			...
			spec: &OverrideSpec{Emergency: false},
			expectedPolicies: []string{"development-policy"},
			expectedCount:    1,
		},
		{
			name: "emergency override",
			...
			spec: &OverrideSpec{Emergency: true},
			expectedPolicies: []string{"production-policy", "emergency-policy"},
			expectedCount:    2,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			matching := manager.findMatchingPolicies(context.TODO(), tt.placement, tt.spec)
			
			if len(matching) != tt.expectedCount {
				t.Errorf("Expected %d matching policies, got %d", tt.expectedCount, len(matching))
			}

			matchingNames := make(map[string]bool)
			for _, policy := range matching {
				matchingNames[policy.Name] = true
			}

			for _, expected := range tt.expectedPolicies {
				if !matchingNames[expected] {
					t.Errorf("Expected policy %s to match", expected)
				}
			}
		})
	}
...
func TestOverrideRuleApplication(t *testing.T) {
	manager := &DecisionOverride{}

	decision := &placementv1alpha1.PlacementDecision{
		Spec: placementv1alpha1.PlacementDecisionSpec{
			Clusters: []placementv1alpha1.ClusterDecision{
				{ClusterName: "cluster-1", Weight: 100},
				{ClusterName: "cluster-2", Weight: 75},
			},
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			rule: &OverrideRule{
				Type:      OverrideRuleTypeClusterSelection,
				Operation: OverrideOperationAdd,
				Value:     []string{"cluster-3"},
			},
			expectedClusters: []string{"cluster-1", "cluster-2", "cluster-3"},
		},
		{
			name: "remove cluster",
			rule: &OverrideRule{
				Type:      OverrideRuleTypeClusterSelection,
				Operation: OverrideOperationRemove,
				Value:     []string{"cluster-1"},
			},
			expectedClusters: []string{"cluster-2"},
		},
		{
			name: "emergency drain",
			rule: &OverrideRule{
				Type:      OverrideRuleTypeEmergencyDrain,
				Operation: OverrideOperationDrain,
				Value:     []string{"cluster-1"},
			},
			expectedClusters: []string{"cluster-2"},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			testDecision := decision.DeepCopy()
			spec := &OverrideSpec{}

			err := manager.applyRule(context.TODO(), testDecision, tt.rule, spec)
			if err != nil {
				t.Errorf("Failed to apply rule: %v", err)
			}

			if len(testDecision.Spec.Clusters) != len(tt.expectedClusters) {
				t.Errorf("Expected %d clusters, got %d", len(tt.expectedClusters), len(testDecision.Spec.Clusters))
			}

			actualClusters := make([]string, len(testDecision.Spec.Clusters))
			for i, cluster := range testDecision.Spec.Clusters {
				actualClusters[i] = cluster.ClusterName
			}

			for i, expected := range tt.expectedClusters {
				if i >= len(actualClusters) {
					t.Errorf("Expected cluster %s at index %d, but not enough clusters", expected, i)
					continue
				}
				if actualClusters[i] != expected {
					t.Errorf("Expected cluster %s at index %d, got %s", expected, i, actualClusters[i])
				}
			}
		})
	}
...
func BenchmarkFindMatchingPolicies(b *testing.B) {
	manager := &DecisionOverride{
		overridePolicies: make(map[string]*OverridePolicy),
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = manager.findMatchingPolicies(context.TODO(), placement, spec)
	}
}