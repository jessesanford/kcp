...
// WorkloadPlacementInterface defines the interface that WorkloadPlacement types must implement
...
// GetTargetClusters returns the selected target clusters
func (w *WorkloadPlacement) GetTargetClusters() []string {
	return w.Status.SelectedClusters
}

// GetStrategy returns the placement strategy
func (w *WorkloadPlacement) GetStrategy() PlacementPolicy {
	return w.Spec.PlacementPolicy
}

// IsPlaced returns true if workloads have been placed
func (w *WorkloadPlacement) IsPlaced() bool {
	for _, cond := range w.Status.Conditions {
		if cond.Type == "Placed" && cond.Status == "True" {
			return true
		}
	}
	return false
...
// GetPlacedWorkloads returns the list of placed workloads
func (w *WorkloadPlacement) GetPlacedWorkloads() []PlacedWorkload {
	return w.Status.PlacedWorkloads
}

// GetLastPlacementTime returns the timestamp of the last placement decision
func (w *WorkloadPlacement) GetLastPlacementTime() *metav1.Time {
	return w.Status.LastPlacementTime
}

// ValidateWorkloadPlacement validates a WorkloadPlacement object
func ValidateWorkloadPlacement(wp *WorkloadPlacement) error {
	if wp.Spec.WorkloadSelector.LabelSelector == nil && len(wp.Spec.WorkloadSelector.WorkloadTypes) == 0 {
		return fmt.Errorf("workload selector must specify either labelSelector or workloadTypes")
	}
	if wp.Spec.NumberOfClusters != nil && *wp.Spec.NumberOfClusters < 1 {
		return fmt.Errorf("numberOfClusters must be at least 1")
	}
	return nil
}

// SetDefaults sets default values for WorkloadPlacement
func (w *WorkloadPlacement) SetDefaults() {
	if w.Spec.PlacementPolicy == "" {
		w.Spec.PlacementPolicy = PlacementPolicyRoundRobin
	}
	if w.Spec.NumberOfClusters == nil {
		var defaultCount int32 = 1
		w.Spec.NumberOfClusters = &defaultCount
	}
}
