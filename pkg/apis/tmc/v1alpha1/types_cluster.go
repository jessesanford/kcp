...
// ClusterRegistrationInterface defines the interface that ClusterRegistration types must implement
...
// GetLocation returns the geographical location of the cluster
func (c *ClusterRegistration) GetLocation() string {
	return c.Spec.Location
}

// GetCapabilities returns the detected capabilities of the cluster
func (c *ClusterRegistration) GetCapabilities() []string {
	if c.Status.Capabilities == nil {
		return nil
	}
	return c.Status.Capabilities.Features
}
...
// GetEndpoint returns the cluster endpoint information
func (c *ClusterRegistration) GetEndpoint() ClusterEndpoint {
	return c.Spec.ClusterEndpoint
}

// GetLastHeartbeat returns the timestamp of the last successful heartbeat
func (c *ClusterRegistration) GetLastHeartbeat() *metav1.Time {
	return c.Status.LastHeartbeat
}

// ValidateClusterRegistration validates a ClusterRegistration object
func ValidateClusterRegistration(cr *ClusterRegistration) error {
	if cr.Spec.Location == "" {
		return fmt.Errorf("location is required")
	}
	if cr.Spec.ClusterEndpoint.ServerURL == "" {
		return fmt.Errorf("cluster endpoint serverURL is required")
	}
	// Validate URL format
	if _, err := url.Parse(cr.Spec.ClusterEndpoint.ServerURL); err != nil {
		return fmt.Errorf("invalid serverURL: %v", err)
	}
	return nil
}

// SetDefaults sets default values for ClusterRegistration
func (c *ClusterRegistration) SetDefaults() {
	if c.Spec.ClusterEndpoint.TLSConfig == nil {
		c.Spec.ClusterEndpoint.TLSConfig = &TLSConfig{
