...
import (
	...
	syncerlogging "github.com/kcp-dev/kcp/pkg/syncer/logging"
...
func (c *Controller) process(ctx context.Context, gvr schema.GroupVersionResource, key string) (retryAfter *time.Duration, err error) {
	...
	if upstreamNamespace != "" {
		...
		if len(downstreamNamespaces) == 1 {
			...
			logger.WithValues(syncerlogging.DownstreamName, namespace.GetName()).V(4).Info("Found downstream namespace for upstream namespace")
	...
	logger = logger.WithValues(syncerlogging.DownstreamNamespace, downstreamNamespace)
...
func (c *Controller) applyToDownstream(ctx context.Context, gvr schema.GroupVersionResource, downstreamNamespace string, upstreamObj *unstructured.Unstructured) error {
	...
	logger = logger.WithValues(syncerlogging.DownstreamName, transformedName)
