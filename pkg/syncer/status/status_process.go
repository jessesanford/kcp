...
func (c *Controller) process(ctx context.Context, gvr schema.GroupVersionResource, key string) error {
	...
	if strings.HasPrefix(downstreamNamespace, syncerlogging.SyncerIDPrefix) {
		// skip syncer namespace
		return nil
	}

	logger = logger.WithValues(syncerlogging.DownstreamNamespace, downstreamNamespace, syncerlogging.DownstreamName, downstreamName)
