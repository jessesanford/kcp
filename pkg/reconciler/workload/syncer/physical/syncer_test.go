...
func TestNewPhysicalSyncer(t *testing.T) {
	tests := map[string]struct {
		...
		"valid configuration": {
			cluster:       &tmc.SimpleClusterTarget{Name: "test-cluster"},
...
func TestWorkloadStatusPhases(t *testing.T) {
	phases := []tmc.WorkloadPhase{
		tmc.WorkloadPhasePending,
		tmc.WorkloadPhaseDeploying,
		tmc.WorkloadPhaseReady,
		tmc.WorkloadPhaseDegraded,
		tmc.WorkloadPhaseFailed,
		tmc.WorkloadPhaseTerminating,
		tmc.WorkloadPhaseUnknown,
	}
	
	expectedPhases := []tmc.WorkloadPhase{
		"Pending",
		"Deploying", 
		"Ready",
		"Degraded",
...
func TestSyncEventTypes(t *testing.T) {
	eventTypes := []tmc.SyncEventType{
		tmc.SyncEventStarted,
		tmc.SyncEventCompleted,
		tmc.SyncEventFailed,
		tmc.SyncEventSkipped,
	}
	
	expectedTypes := []tmc.SyncEventType{
		"SyncStarted",
		"SyncCompleted",
		"SyncFailed", 
		"SyncSkipped",
...
func TestPrepareWorkloadForCluster(t *testing.T) {
	...
	cluster := &tmc.SimpleClusterTarget{}
...
func TestSyncEventCreation(t *testing.T) {
	event := &tmc.SyncEvent{
		...
		Workload:  tmc.WorkloadRef{Name: "test-workload"},
		Timestamp: time.Now(),
		Message:   "test message",
	}
	
	if event.Type != tmc.SyncEventStarted {
		t.Errorf("expected event type %s, got %s", tmc.SyncEventStarted, event.Type)
	}
...
type mockEventHandler struct {
	events []tmc.SyncEvent
}

func (m *mockEventHandler) HandleEvent(ctx context.Context, event *tmc.SyncEvent) error {
